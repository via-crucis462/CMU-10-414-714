...
class BroadcastTo(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        for axis in range(len(self.shape)):
            input_axis = axis - (len(self.shape) - len(input_shape))
            if input_axis < 0 or input_shape[input_axis] != self.shape[axis]:
                axes.append(axis)
        summed = summation(out_grad, axes=tuple(axes)) if axes else out_grad
        return reshape(summed, input_shape)
        ### END YOUR SOLUTION
...
class Summation(TensorOp):
    ...
    def compute(self, a):
        ### BEGIN YOUR SOLUTION
        return array_api.sum(a) if self.axes is None else array_api.sum(a, axis=self.axes)
        ### END YOUR SOLUTION
